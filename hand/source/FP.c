...
#ifdef INPUT_TXT
sprintf(filen, "%sinput/inputintc.txt", PATH);
finp=fopen(filen, "r");
if(finp==NULL)
{
	printf("Error al abrir el archivo, pendejo\n");
	return 1;
	
}
	saltar_com(finp);
fscanf(finp, "%d", &NumAng);
printf("El numero de angulos a propagar son = %d\n", NumAng);
...
{sprintf(filen, "%sinput/inputintc.txt", PATH);
	...
	fclose(finp);
	sprintf(filen, "%sinput/inputdouc.txt", PATH);
	finp=fopen(filen, "r");
					saltar_com(finp);
...
radio=deltay/2;
				...
				sprintf(filen, "%sres/matriz_objeto.txt", PATH);
			finp=fopen(filen, "w");
...
FILE *apertura_archivo(void)
{
   ...
   sprintf(namefile, "%sres/Result_LFT", PATH);	
...
void propagacion(struct param targ) /* Pasarlo como referencia ¿?, en ese caso tendría que definir variables con ->*/
{
	...
	Niter=(int)(targ.dix*targ.deltaX/targ.deltaPaso); /*Numero de pasos de propagación del rayo a través del objeto */
		...
		sprintf(filen, "%sres/resultadosFPredux.txt", PATH);
		fout=fopen(filen, "w");
	for(i=0; i<targ.N; i++)
	{
	...
	*(pos0+1)=betarand;
	...
	num_rand=2*targ.angma*i/targ.N;
	...
	*pos0+=*(pos0+1)*targ.d; /*Prop. desde la fuente al objeto */
	...
	fclose(fout);
				...
				fprintf(pipeplot, "call '%sgp/LFTT.gp' %d\n", PATH, (numout-1));
				pclose(pipeplot);
				sprintf(filen, "%sres/matresul.txt", PATH);
				fui=fopen(filen, "w");
				for(i=0; i<numout; i++)
				fprintf(fui, "%d %d\n", resul[i][0], resul[i][1] );
	fclose(fui);
