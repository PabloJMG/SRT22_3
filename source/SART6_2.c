	 ...
	 for(cont2=0; cont2<NumPix; cont2++){
			...
			nth=0;
						 ...
						 {	
							...
							if(coe[cont][cont2].a[pox][poy]>0.00001) /* Si esto se cumple gamma no debería NO ser 0 ¿?*/
							{
 ...
 sumpix=0;
	 ...
	 {	
		...
		{//~ printf("cont2=%d, numhilo=%d\n", cont2, omp_get_thread_num());
			for(cont3=(factordim-1)*dimy/2; cont3<(factordim+1)*dimy/2; cont3++)
			{		if(gamma[cont].a[cont2][cont3] != 0) /* Este if es conflictivo e igual sobra según la definición de gamma */
				...
				for(cont5=0; cont5<(nonum[cont][cont4].nonnum); cont5++)
				{
					...
					cant+=nonum[cont][cont4].as[cont5]*(x[nonum[cont][cont4].pixeles[0][cont5]][nonum[cont][cont4].pixeles[1][cont5]]);
						printf("cant = %f, as = %f, x = %f\n", cant, nonum[cont][cont4].as[cont5], x[nonum[cont][cont4].pixeles[0][cont5]][nonum[cont][cont4].pixeles[1][cont5]]);//~ printf("Suma a cantidad = %lf mult por %lf\n", nonum[cont][cont4].as[cont5], (x[nonum[cont][cont4].pixeles[0][cont5]][nonum[cont][cont4].pixeles[1][cont5]]) );	
				}
				//~ printf("NumAng = %d, NumDet = %d, cant=%lf, eje x =%d, eje y =%d, coef=%lf, beta = %f, valor del  pixel = %f nonum=%f\n", cont, cont4, cant, cont2, cont3,  coe[cont][cont4].a[cont2][cont3], betar[cont][cont4],nonum[cont][cont4].as[cont5] ,x[nonum[cont][cont4].pixeles[0][cont5]][nonum[cont][cont4].pixeles[1][cont5]]);   /* No muy seguro de los dos ultimos %%% */
				printf("cant = %f\n\n", cant);
				
				//~ getchar();
				//~ printf("coe = %lf\n", coe[cont][cont4].a[cont3][cont2]);
