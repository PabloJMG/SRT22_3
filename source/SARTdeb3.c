...
for(cont2=(factordim-1)*dimx/2; cont2<(factordim+1)*dimx/2; cont2++)
		{
			for(cont3=(factordim-1)*dimy/2; cont3<(factordim+1)*dimy/2; cont3++)
			{
							...
							//~ pox=trunc(posx+factordim*dimx/2);
							//~ poy=trunc(posy+factordim*dimy/2);
					//~ printf("c2 = %d, c3 = %d, px = %d, py = %d\n", cont2, cont3, pox, poy);
					//~ if((gamma[cont].a[pox][poy] > 0.0001)  && (coe[cont][cont4].a[pox][poy] > 0.0001))
					//~ {
								//~ nonum[cont][cont4].pixeles[0][nth]=pox;
								//~ nonum[cont][cont4].pixeles[1][nth]=poy;
								//~ nonum[cont][cont4].as[nth]=coe[cont][cont4].a[pox][poy];
								//~ nth++;
					//~ }
					//~ getchar();
					
						if((gamma[cont].a[cont2][cont3] > 0.0001)  && (coe[cont][cont4].a[cont2][cont3] > 0.0001))
					{
...
//~ getchar();
 ...
 sumpix=0;
	 ...
	 //~ {	
		...
		printf("cont, cont2, cont3 = %d %d %d gamma a dividir = %f\n", cont, cont2, cont3, gamma[cont].a[cont2][cont3]);
		//~ getchar();
