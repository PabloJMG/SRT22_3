...
struct nopze nonm[limx][limy]; //Podría y debería ser factordim*factordim veces más pequeña esta matriz de structs
...
for(cont=0; cont<limx; cont++)
{
	for(cont2=0; cont2<limy; cont2++)
	{
		...
		nonm[cont][cont2].ang=calloc(NumAng, sizeof(int));
		nonm[cont][cont2].nonnum=0;
	}
...
//~ getchar();
 for(cont=0; cont<NumAng; cont++)
	...
	printf("End of gammas, cero = %d de %d ahora hay un getchar()", gammcon, dimx*dimy*NumAng);
//~ getchar();
i=0;
deltaf=1;
...
for(cont2=(factordim-1)*dimx/2; cont2<(factordim+1)*dimx/2; cont2++)
		{
			for(cont3=(factordim-1)*dimy/2; cont3<(factordim+1)*dimy/2; cont3++)
			{
					...
					{
								nonum[cont][cont4].pixeles[0][nth]=cont2;
								nonum[cont][cont4].pixeles[1][nth]=cont3;
								nonum[cont][cont4].as[nth]=coe[cont][cont4].a[cont2][cont3];
								nth++;
					//~ printf("cont %d cont2 %d cont3 %d gamma %f coe %f\n",cont, cont2, cont3, gamma[cont].a[cont2][cont3], coe[cont][cont4].a[cont2][cont3]);
							//~ getchar();
					}
...
for(cont2=(factordim-1)*dimx/2; cont2<(factordim+1)*dimx/2; cont2++)	
{
		...
		for(cont3=(factordim-1)*dimy/2; cont3<(factordim+1)*dimy/2; cont3++)
		{
			...
			for(cont=0; cont<NumAng; cont++)
			{
				if(gamma[cont].a[cont2][cont3] > 0.0001)
				{
					nonm[cont2][cont3].ang[ng]=cont;
					
					ng++;
				}
			}
		nonm[cont2][cont3].nonnum=ng;
		}
	 ...
	 //~ {	
		 ...
		 //~ #pragma omp parallel
			...
			for(cont2=(factordim-1)*dimx/2; cont2<(factordim+1)*dimx/2; cont2++) /*Cambiados limites aqui */
			{
				...
				for(cont3=(factordim-1)*dimy/2; cont3<(factordim+1)*dimy/2; cont3++)
				{    difgam=0;
					 for(cont=0; cont<nonm[cont2][cont3].nonnum; cont++) /*Qué conts cambiar, todos ¿? */
						{	
							diferencia=0;
								for(cont4=0; cont4 < NumPix; cont4++)
								{
										...
										diferencia+=(b[nonm[cont2][cont3].ang[cont]][cont4]-cant)*((coe[nonm[cont2][cont3].ang[cont]][cont4].a[cont2][cont3]))/(betar[nonm[cont2][cont3].ang[cont]][cont4]);
					
								} /* Final del bucle que recorre todos los detectores  */
			

							difgam+=diferencia/(gamma[nonm[cont2][cont3].ang[cont]].a[cont2][cont3]);
						
						}	/*Final bucle NumAng*/
				
							x[cont2][cont3]+=lambda*difgam; /* update de un pixel -- ESTO MEGA CAMBIO MEGA INVENTADA, básicamente lo de dividir entre nonum*/
							deltaf+=diferencia;
