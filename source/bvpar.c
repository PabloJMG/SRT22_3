...
//~ static int numpix[2];
 ...
 #ifdef INPUT_TXT
finp=fopen("/home/pablo/Resultados_C/LFT/input/inputint.txt", "r");
		...
		for(i=0; i<Num_Pix; i++)/*Cuerpo de toda la simulación, proyeccion de cada pixel */
		{
			...
			if(datainp[i][cont] != 0)
			{
				...
				{	
					...
					//~ printf("betarnd = %f, betamin = %f, betamax = %f, i = %d\n", betarand, betamin, betamax, i);
					//~ getchar();
					...
					for(q=0; q<pasx; q++)
					{
						...
						pos1rnd[1]=pos0rnd[1];
						pos0rnd[0]=pos1rnd[0];
						//~ printf("yl = %f, pos = %f, ang = %f, paso = %d, numpix=%d betarbd = %f, betamin = %f, betamax = %f\n", yl, pos0rnd[0], pos0rnd[1], q, i, betarand, betamin, betamax);
						#ifdef SUM_ONE
...
char archivo[100]="/home/pablo/Resultados_C/LFT/res/Resultados_BP.txt";
...
char nombrearchivo[65]="res/Resultados_pixeles.txt";
...
sprintf(nombrearchivo, "/home/pablo/Resultados_C/LFT/res/rayo%d_%d_%d.txt", contador3, contador, contador2);
				...
				double anchura_mall, double *pos, double sinbet, double cosbet, int *numpix)
{
	...
	int pix2[2];
		...
		*numpix=rint(*pix2+factodim/2*dix);
		*(numpix+1)=rint(*(pix2+1)+factodim/2*diy);
