...
//Completamente cambiado el bucle while

#define PATH "/home/pablo/Resultados_C/FP_SART"
#define pi 3.141592
#define DOSPI
#define COEF_COM
...
int NumPix, gammcon, cont, cont2, cont3, cont4, cont5, cont6, dimx, dimy, pixx, pixy, NumAng, i, valu, dimtotal, NumRayos, nth, ng, limx, limy, con2, con3, pox, poy;
float beta, cosbeta, difgam, factora, sinbeta, deltaang, lambda, deltax, deltay, deltaf, posx, posy, posx2, posy2, val, diferencia, cant, sumpix, factorpos, prue1, prue2;
double angulos[10];
char filename[100];
FILE *finp;
...
//Carga parámetros de input (NumAng, NumPix) 
...
struct nonzer n_coe[NumAng][NumPix]; //Guarda elementos no nulos de la matriz de coeficientes (Para cada ang y pixel, pos en eje x e y, valor)
struct nopze n_gamm[limx][limy]; //Podría y debería ser factordim*factordim veces más pequeña esta matriz de structs
...
for(cont=0; cont<NumAng; cont++)
{
	...
	for(cont2=0; cont2<NumPix; cont2++)
	{
		...
		for(cont3=0; cont3<limx; cont3++)
		{
			...
			{
				coe[cont][cont2].a[cont3][cont4]=0.f;
...
for(cont=0; cont<limx; cont++)
{
	for(cont2=0; cont2<limy; cont2++)
	{
		...
		n_gamm[cont][cont2].ang=calloc(NumAng, sizeof(int));
		n_gamm[cont][cont2].nonnum=0;
	}
...
//~ Carga_archivo(NumAng, NumPix, b);	
 for(cont=0; cont<NumAng; cont++) /*  Cargar b (esto está mal)*/
 {
	 ...
	 finp=apertura_archivo(cont);
		 ...
		 {
				fscanf(finp, "%d", &valu);
				b[cont][cont2]=log(1+valu); 
				if(cont==0)  // AQUI ARREGLO MÁGICO
				b[cont][cont2]=2*log(1+valu);
...
/* Reserva de mem para pixeles.n_coe y as.n_coe  */
		...
		for(cont=0; cont<NumAng; cont++)
		{
			...
			{
				 n_coe[cont][cont2].as=malloc(dimtotal*sizeof(float));
				 n_coe[cont][cont2].pixeles=malloc(2*sizeof(int *));
				 for(cont3=0; cont3<2; cont3++)
				 
				 n_coe[cont][cont2].pixeles[cont3]=malloc(dimtotal*sizeof(int));
				
			}
		}

//Inicialización de matriz de coeficientes a
#ifdef COEF_COM
 for(cont=0; cont<NumAng; cont++) 
 { 		
	 #ifdef DOSPI 
							 ...
							 //~ posy2=posy-factordim*dimy/2;
								 ...
								 coe[cont][cont2].a[pixx+1][pixy+1]+=(deltax+deltay)/factora;
							 		//~ printf("posx=%f, posy=%f, pixx = %d, pixy=%d", posx, posy, pixx, pixy);
										 ...
										 //~ for(cont4=0; cont4<dimy; cont4++) /*Este for sirve para algo¿?*/
										 //~ {
 ...
 #endif
//~ printf("guarda mat coef en file");
//Guarda matriz de coeficientes en archivo
 for(cont=0; cont<NumAng; cont++)
 {
	 for(cont2=0; cont2<NumPix; cont2++)
	{ //~ { finp=apertura_archivo2(cont, cont2);
...
//Suma de la matriz de coeficientes para un mismo ángulo y un mismo detector.
for(cont=0; cont<NumAng; cont++)
 {
		for(cont2=0; cont2<NumPix; cont2++)
		{
...
gammcon=0;
//Suma de la matriz de coeficientes para un mismo ángulo y un mismo píxel en la imagen.
 for(cont=0; cont<NumAng; cont++)
	{
	...
	//~ printf("End of gammas, cero = %d de %d ahora ahora hay un getchar()", gammcon, dimx*dimy*NumAng);
//~ getchar();
i=0;
deltaf=1;
...
//Recorre matriz de coeficientes buscando los no nulos.
for(cont=0; cont<NumAng; cont++)
{
		...
		for(cont2=(factordim-1)*dimx/2; cont2<(factordim+1)*dimx/2; cont2++)
			{
					for(cont3=(factordim-1)*dimy/2; cont3<(factordim+1)*dimy/2; cont3++)
					{												
							if(coe[cont][cont4].a[cont2][cont3] > 0.0001)
								{
									n_coe[cont][cont4].pixeles[0][nth]=cont2;
									n_coe[cont][cont4].pixeles[1][nth]=cont3;
									n_coe[cont][cont4].as[nth]=coe[cont][cont4].a[cont2][cont3];
									nth++;
				 ...
				 n_coe[cont][cont4].nonnum= nth;  //Antes ponía n_coe[cont][cont2] ¿?
			}
...
for(cont2=(factordim-1)*dimx/2; cont2<(factordim+1)*dimx/2; cont2++)	
{
		...
		for(cont3=(factordim-1)*dimy/2; cont3<(factordim+1)*dimy/2; cont3++)
		{
			...
			for(cont=0; cont<NumAng; cont++)
			{
				if(gamma[cont].a[cont2][cont3] > 0.0001)
				{
					n_gamm[cont2][cont3].ang[ng]=cont;
					ng++;
				}
			}
			n_gamm[cont2][cont3].nonnum=ng;
	 ...
	 //~ {	
		 ...
		 //~ #pragma omp parallel
			...
			for(cont2=(factordim-1)*dimx/2; cont2<(factordim+1)*dimx/2; cont2++) /*Cambiados limites aqui */
			{
				...
				for(cont3=(factordim-1)*dimy/2; cont3<(factordim+1)*dimy/2; cont3++)
				{    difgam=0;
					 for(cont=0; cont<n_gamm[cont2][cont3].nonnum; cont++) /* ¿? */
						{	
							diferencia=0;
								for(cont4=0; cont4 < NumPix; cont4++)
								{
										cant=0;	
												...
												for(cont5=0; cont5<(n_coe[cont][cont4].nonnum); cont5++)
												{										
													cant+=n_coe[cont][cont4].as[cont5]*(x[n_coe[cont][cont4].pixeles[0][cont5]][n_coe[cont][cont4].pixeles[1][cont5]]);												
												}
																															
										diferencia+=(b[n_gamm[cont2][cont3].ang[cont]][cont4]-cant)*((coe[n_gamm[cont2][cont3].ang[cont]][cont4].a[cont2][cont3]))/(betar[n_gamm[cont2][cont3].ang[cont]][cont4]);
					
								} /* Final del bucle que recorre todos los detectores  */
			

							difgam+=diferencia/(gamma[n_gamm[cont2][cont3].ang[cont]].a[cont2][cont3]);
						
						}	/*Final bucle NumAng*/
				
							x[cont2][cont3]+=lambda*difgam; /* update de un pixel */
...
fprintf(pipeplot, "set output 'img/NumAng_%d_Numpix_%d,dim_%dx%d__%d.png' \n", NumAng, NumPix, dimx, dimy, archtime);
