...
//~ getchar();
		 ...
		 float *x=calloc(dimtotal, sizeof(float));
		 float *xantes=calloc(dimtotal, sizeof(float));
...
for(cont2=0; cont2<NumPix; cont2++)
{
	for(cont3=0; cont3<dimx; cont3++)
	{
		for(cont4=0; cont4<dimy; cont4++)
		{
					pox=cont3+(factordim-1)*dimx/2; /* ¿? ¿?¿?¿?¿?¿?¿?*/
					poy=cont4+(factordim-1)*dimy/2;
			//~ *(betar+cont*NumPix+cont2)+=(*(a+cont*NumPix*dimtotal+cont2*dimtotal+cont3*dimy+cont4))*(*(a+cont*NumPix*dimtotal+cont2*dimtotal+cont3*dimy+cont4));
				*(betar+cont*NumPix+cont2)+=(coe[cont][cont2].a[pox][poy]);
 ...
 for(cont=0; cont<NumAng; cont++)
	{
		for(cont2=0; cont2<dimx; cont2++)
		{
			...
			
			{
				...
				{		pox=cont2+(factordim-1)*dimx/2; /* ¿? ¿?¿?¿?¿?¿?¿?*/
					poy=cont3+(factordim-1)*dimy/2;
					*(gamma+cont*dimtotal+pox*dimy+poy)+=(coe[cont][cont2].a[pox][poy]); /*Esta bien esta inicializacion¿? */
...
deltaf=1;
 ...
 sumpix=0;
	 ...
	 {	
		...
		for(con2=0; con2<dimx; con2++) /*Cambiados limites aqui */
		{//~ printf("cont2=%d, numhilo=%d\n", cont2, omp_get_thread_num());
			for(con3=0; con3<dimy; con3++)
			{
				for(cont4=0; cont4 < NumPix; cont4++)
				{
					cant=0;
	   ...
	   /*Aqui habia dos for nesteados que he eliminado*/
				...
				printf("NumAng = %d, cant=%lf, ejex=%d, ejex mod=%d, ejey =%d, ejey mod=%d, coef=%lf\n", cont, cant, con2, cont2, con3, cont3, coe[cont][cont2].a[cont3][cont4]);
				//~ getchar();
			...
			*(x+cont2*dimy+cont3)+=lambda*diferencia/(*(gamma+cont*dimtotal+cont2*dimy +cont3)); /* update de un pixel */
			...
			printf("diferencia =%f, pixel nuevo =%f, pixel %d %d, lambda=%f, gamma =%f, beta = %lf, b=%lf\n", lambda*diferencia/(*(gamma+cont*dimtotal+cont2*dimy +cont3)), *(x+cont2*dimy+cont3), cont2, cont3, lambda, *(gamma+cont*dimtotal+cont2*dimy +cont3), *(betar+cont*NumPix+cont4), *(b+cont*NumPix+cont4));
