...
int NumPix, cont, cont2, cont3, cont4, cont5, cont6, dimx, dimy, pixx, pixy, NumAng, i, valu, dimtotal, NumRayos, nth;
...
factorpos=(float)factordim*dimy/NumPix;  /* factordim incluido, necesario ¿?*/
printf("factorpos=%f\n", factorpos);
//~ getchar();
...
float *b = calloc(NumPix*NumAng, sizeof(float)); /*cambiado hoy */
 float *x=calloc(factordim*dimx*dimy, sizeof(float));
			 ...
			 *(a+cont*NumPix*factordim*factordim*dimx*dimy+pixy*factordim*factordim*dimx*dimy+pixy+cont3*dimy)+=1;
			 ...
			 for(cont4=0; cont4<dimy; cont4++)
			 {
				//~ printf( "cont3=%d, cont4=%d y a=%f\n ", cont3, cont4, *(a+cont*NumPix*dimx*dimy+cont2*dimx*dimy+cont4+cont3*dimy));
							 if(*(a+cont*NumPix*dimx*dimy+cont2*dimx*dimy+cont4+cont3*dimy) !=0)
							 {
	 ...
	 {	
		...
		{//~ printf("cont2=%d, numhilo=%d\n", cont2, omp_get_thread_num());
			...
			{//~ {	printf("nozeros=%d\n", nonum[cont][cont4].nonnum);
				...
				diferencia+=(*(b+cont*NumPix+cont4)-cant)*(*(a+cont*NumPix*dimtotal+cont4*dimtotal+cont2*dimy+cont3))/(*(betar+cont*NumPix+cont4)); /* Aquí mirar  ---- (b-a*x)*a_{j}/b_j */
