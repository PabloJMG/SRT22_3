...
{
int i, cont, dimx, dimy, factordim, NumAdq,rc, dimtotal, a, b;
int array_inp[4];
double array_inp2[6];
...
fu1=fopen("/home/pablo/Resultados_C/LFT/input/inputint.txt", "r");
...
fu1=fopen("/home/pablo/Resultados_C/LFT/input/inputdou.txt", "r");
...
//~ dimx=(int)(array_inp2[0]/array_inp[1]); /*Necesario¿?*/
//~ dimy=(int)(array_inp2[0]/array_inp[2]);
//~ d=0.05;
...
//~ objeto=trataobj(fu1, par[0].factordim,  par[0].dix, par[0].diy, par[0].factordim*par[0].dix/20, par[0].factordim*par[0].diy/20);
	for(a=0; a<(par[0].factordim*par[0].dix); a++)
	{
		...
		{		
			*(objeto+a*par[0].factordim*par[0].diy+b)=1000;
...
char nombrearchivo[75]="Resultados_pixeles.txt";

sprintf(nombrearchivo, "/home/pablo/Resultados_C/LFT/res/angulo_%d.txt", contador);
...
{
	double num_rand, betarnd, deltadis, deltaP, Niter, cosang, sinang,  mu;
	double pos0[2], pos1[2];
	int i, j, pixx, pixy, num_pix, NRayos, cont, deltaang, pi1, pi2;
	int *pix=malloc(2*sizeof(int));
	int *pix2=malloc(2*sizeof(int));
	struct param *mispar;
...
printf("El número de píxel es %d y pos del rayo es %lf y angulo %d\n", num_pix, *pos0-factor*anchura/2+anchura/2, cont);
